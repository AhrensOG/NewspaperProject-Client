  ...
  useEffect(() => {
    try {
      const getCategories = async () => {
        ...
        // setCookies(cookie);
        const randomCategories = res.data?.map(
          () => res?.data[Math.floor(Math.random() * res?.data?.length)]
        );
        const filteredRandomCategories = randomCategories.filter(
          ...
          <div className="hidden sm:flex flex-nowrap sm:gap-4 md:gap-6 lg:gap-8 h-full items-center w-full">
            {categories.length > 0 ? (
              <div className="lg:hidden flex flex-row justify-center items-center">
                ...
                <span
                  onClick={handleSidebar}
                  className="text-start text-base font-roboto uppercase text-black py-2 px-2 font-medium h-15 hover:cursor-pointer"
                >
